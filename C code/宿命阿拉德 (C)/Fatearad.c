...
void Manage() {
	while (1) {
		...
		SetPosition(55, 17);
		printf("阿拉德大陆总人口(按x键查看)");
		SetPosition(55, 19);
		printf("0.返回上一层");

		char key;
		fflush(stdin);
		while ((key = getch()) != '1' && key != '2' && key != '3' && key != '4' && key != '5' && key != '0' && key != 'x' && key != 'X');
		fflush(stdin);
		...
		switch (key) {
		    ...
		    case 'x':
		    case 'X':   SetColor(14, 0); SetPosition(20, 7);
                        printf("阿拉德大陆共有 -%d- 位勇士", totalQuantity);
                        SetColor(10, 0);getch();
                        continue;
		    case '0':   return;
		}
...
void Dispose() {
	while (1) {
		...
		switch (key) {
		    case '1':   Sort(Target, 1);   break;
		    case '2':   Sort(Target, 2);   break;
		    case '3':   Sort(Target, 3);   break;
...
int GameStart() {
	...
	char key;
	fflush(stdin);
	while ((key = getch()) != '1' && key != '2' && key != 27 && key != '0');
	fflush(stdin);
...
void Register() {
	...
	switch (c) {
	    case '1':   strcpy(currPlayer->role, "鬼剑士(男)");     break;
	    case '2':   strcpy(currPlayer->role, "格斗家(女)");     break;
	    case '3':   strcpy(currPlayer->role, "神枪手(男))");    break;
	    case '4':   strcpy(currPlayer->role, "圣职者");         break;
	    case '5':   strcpy(currPlayer->role, "神枪手(女) ");    break;
	    case '6':   strcpy(currPlayer->role, "暗夜使者");       break;
	    case '7':   strcpy(currPlayer->role, "格斗家(男)");     break;
	    case '8':   strcpy(currPlayer->role, "守护者");         break;
	    case '9':   strcpy(currPlayer->role, "魔法师(男)");     break;
	    case 'A':
	    case 'a':   strcpy(currPlayer->role, "魔枪士");         break;
	    case 'B':
	    case 'b':   strcpy(currPlayer->role, "鬼剑士(女)");     break;
	    case 'C':
	    case 'c':   strcpy(currPlayer->role, "黑暗武士");       break;
	    case 'D':
	    case 'd':   strcpy(currPlayer->role, "缔造者");         break;
	}
	totalQuantity++;
	system("cls");
}
